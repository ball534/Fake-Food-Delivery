...
export default function DealPrice({
  ...
  const save =
    deal.originalPrice != null ? Math.round((deal.originalPrice - deal.price) * 100) / 100 : 0;
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      <span className="font-extrabold">{money(deal.price)}</span>
      {deal.originalPrice != null && (
        <span className="text-sm text-white/60 line-through">{money(deal.originalPrice)}</span>
      )}
      {save > 0 && (
